...
void PadTeensy::display_fft(uint8_t band_array[], uint8_t nbr_band){
  // SET ALL TO ZERO BUT DO NOT SHOW
  if (_mode == 1){
    for (uint16_t i=0; i<_trellis->pixels.numPixels(); i++){
      _trellis->pixels.setPixelColor(i, 0x000000);
    }
    ...
    for (int i=0;i<nbr_band;i++){
      if (band_array[i] >= 1){
        _trellis->pixels.setPixelColor(12 + i, this->get_color(map(12 + i, 0, _trellis->pixels.numPixels(), 0, 255)));
      }
      if (band_array[i] >= 2){
        _trellis->pixels.setPixelColor(8 + i, this->get_color(map(8 + i, 0, _trellis->pixels.numPixels(), 0, 255)));
      }
      if (band_array[i] >= 3){
        _trellis->pixels.setPixelColor(4 + i, this->get_color(map(4 + i, 0, _trellis->pixels.numPixels(), 0, 255)));
      }
      if (band_array[i] >= 4){
        _trellis->pixels.setPixelColor(0 + i, this->get_color(map(0 + i, 0, _trellis->pixels.numPixels(), 0, 255)));
      }
    }
    _trellis->pixels.show();
  }
