...
#include "Arduino.h"
#include "PadTeensyMicOnly.h"
#include "../GlobalInitMicOnly/GlobalInitMicOnly.h"
#include "Adafruit_NeoTrellis.h"


PadTeensyMicOnly::PadTeensyMicOnly(){
  // INIT INTERUPT PIN
  pinMode(trellisInterruptPin, INPUT);
  _trellis = new Adafruit_NeoTrellis;
}

void PadTeensyMicOnly::init(TrellisCallback (*callback)(keyEvent evt)){
...
void PadTeensyMicOnly::clear_pad(){
...
void PadTeensyMicOnly::display_animation(){
  if (millis() - _previousMillisAnimation > _delayMillis){
    if (_animation_mode){
      //_trellis->pixels.setPixelColor(_pad_id_animation, this->get_color(random(0, 255)));
...
void PadTeensyMicOnly::set_color(uint8_t button_position, uint32_t color){
  _trellis->pixels.setPixelColor(button_position, color);
  _trellis->pixels.show();
}

uint32_t PadTeensyMicOnly::get_color(byte button_position) {
...
TrellisCallback PadTeensyMicOnly::callback_trellis(keyEvent evt){
...
void PadTeensyMicOnly::reset_message(){
  _new_message = false;
}

uint8_t PadTeensyMicOnly::get_message(){
  return _message;
}
